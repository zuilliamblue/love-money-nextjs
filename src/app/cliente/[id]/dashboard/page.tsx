  ...
  useEffect(() => {
    async function carregarDados() {
      ...
      try {
        ...
        const refAss = collection(db, 'usuarios', uid, 'assinaturas');
        const qAss = query(
          refAss,
        ...
        for (const docCart of cartoesSnap.docs) {
          const refGastosCart = collection(db, 'usuarios', uid, 'cartoes', docCart.id, 'gastos');
        ...
        const refBoletos = collection(db, 'usuarios', uid, 'boletos');
        ...
        const refEmp = collection(db, 'usuarios', uid, 'emprestimos');
        const qEmp = query(
          refEmp,
          where('dataPagamento', '>=', tsInicioMes),
          where('dataPagamento', '<=', tsFimMes)
        ...
        const refFin = collection(db, 'usuarios', uid, 'financiamentos');
        const qFin = query(
          refFin,
          where('dataPagamento', '>=', tsInicioMes),
          where('dataPagamento', '<=', tsFimMes)
        ...
        const refPix = collection(db, 'usuarios', uid, 'pix');
        const qPix = query(
          refPix,
          where('dataPagamento', '>=', tsInicioMes),
          where('dataPagamento', '<=', tsFimMes)
        ...
        cartoesSnap.docs.forEach(docCart => {
          const data = docCart.data() as any;
